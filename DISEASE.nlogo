patches-own [
  immunized?  ;; protected from disease
  sick?       ;; caught disease
  dead?       ;; died from disease
  recovering? ;; can no longer get sick
  next-state  ;; 1 is turning sick, 2 is turning dead, 3 is recovering.
  sick-neighbors
]
...
to setup-random
  clear-all
  ask patches
    [ ifelse random-float 1.0 < initial-immune
      [ set-immune ]
...
to set-recovering  ;; patch procedure  cell-blue
  set immunized? false
  set sick? false
  set dead? false
  set recovering? true
...
to go
  ask patches[
    set sick-neighbors count neighbors with [sick?]
    if (not recovering?) and (not sick?) and (not dead?) and (not immunized?)
      [if sick-neighbors > 0 and random-float(0.999) < sickness-chance
        [set next-state 1]
      ]
    if (not recovering?) and (not sick?) and (not dead?)
      [if sick-neighbors > 0 and random-float(0.999) < sickness-chance and random-float(0.999) >= immunity-effectiveness
        [set next-state 1]
      ]
    if (not recovering?) and (sick?) and random-float(0.999) < death-chance
      [set next-state 2]
    if (not recovering?) and (sick?) and not (next-state = 2) and random-float(0.999) < recovery-chance
      [set next-state 3]
  ]
